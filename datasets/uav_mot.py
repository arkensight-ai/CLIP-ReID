...
class UAV_MOT(BaseImageDataset):
    """
    SeaDroneSee2019MOT
    """
    dataset_dir = 'bounding_box_train'
    def __init__(self, root='', verbose=True, test_size=None, **kwargs):
        super(UAV_MOT, self).__init__()
        ...
        if test_size != 'small' :
            ...
            print(f"=> Using full test set with {test_size} images per person")
        else:
            print(f"=> Using small test set with {test_size} images per person")
            self.test_list = osp.join(self.split_dir, 'test_list_small.txt')
